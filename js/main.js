require([], function (){
    ...
    // Random image 边栏顶部随机背景图
    var imageList = yiliaConfig.overlayList.split(','); 
    var id = Math.ceil(Math.random()*(imageList.length-1));
    imageList[id] = imageList[id]=="none"?"linear-gradient(120deg, #155799, #159957)":"url('"+ imageList[id] +"')";
    // PC
    $("#container .left-col .overlay").css({"background-image": imageList[id],"opacity": 1,"background-size":"cover"});
    //Mobile
    $("#container #mobile-nav .overlay").css({"background-image": imageList[id],"opacity": 1,"background-size":"cover"});
    //Table
